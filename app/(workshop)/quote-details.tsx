...
import { Quote, InventoryItem, Vehicle } from '@/types';
    ...
    const loadQuote = async () => {
        ...
        try {
            ...
            if (quoteData) {
                ...
                if (quoteData.jobId) {
                    try {
                        const job = await firebaseService.getJob(quoteData.jobId);
                        if (job?.vehicleId) {
                ...
                <body>
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-block" style="max-width: 300px;">
                            <p class="info-label" style="text-align: right;">QUOTE DETAILS</p>
                            <div class="info-date"><span>Issued:</span><strong>${format(quote.createdAt, 'MMM dd, yyyy')}</strong></div>
                            ${quote.sentAt ? `<div class="info-date"><span>Sent:</span><strong>${format(quote.sentAt, 'MMM dd, yyyy')}</strong></div>` : ''}
                        </div>
                    </div>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                ...
                {vehicle && (
                    <View style={styles.card}>
                        ...
                        <Text style={styles.customerName}>{vehicle.year} {vehicle.make} {vehicle.model}</Text>
                        {vehicle.licensePlate ? (
                            <Text style={styles.customerDetail}>Plate: {vehicle.licensePlate}</Text>
                        ) : null}
                    </View>
                )}
